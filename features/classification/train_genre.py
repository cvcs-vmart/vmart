from dataloader_genre import WikiArtDataset
...
def train(model, train_loader, test_loader):
    ...
    # wandb
    wandb.init(project="DINOcls", entity="bilardellodavides-vmart", name="dino-genre-training-CLS")
    ...
    for e in tqdm(range(epochs)):
        ...
        total_loss = 0.0
        for data, genre_gt in train_loader:

            # sposto i dati sul device
            data = data.to(device)
            genre_gt = genre_gt.to(device)

            optimizer.zero_grad()

            outputs = model(data)
            loss = criterion(outputs, genre_gt)
            ...
            all_labels.extend(genre_gt.cpu().numpy())
        ...
        with torch.no_grad():
            for data, genre_gt in test_loader:
                data = data.to(device)
                genre_gt = genre_gt.to(device)
...
if __name__ == '__main__':
    ...
    model = DINOStyle(in_channels=384, num_classes=len(dataset.genres)).to(device)
